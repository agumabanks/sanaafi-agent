...
import '../../../../common/widgets/hero_dialogue_route_widget.dart';
import '../controllers/AddGuarantorController.dart';
import 'AddGuarantorPage.dart';
import 'clientQr.dart';
import '../widgets/clientAddPhoto.dart';
import '../../loans/loans/controllers/loanController.dart';
import '../../loans/loans/screens/newloanForm.dart';
import '../../../auth/controllers/auth_controller.dart';
import '../../../auth/widgets/qr_code_popup_widget.dart';
...
import '../../../savings/screens/savings_detail_screen.dart';
                                          ...
                                          Container(
                                                              ...
                                                              child: ClipRRect(
                                                                ...
                                                                child: CustomImageWidget(
                                                                  fit: BoxFit.cover,
                                                                  // https://app.sanaa.co/storage/app/public/clients/photos/WswWAGeAnVbHtQy8eoSoTT4eVEwoC5Xn9Wmpperp.jpg
                                                                  image: "https://app.sanaa.co/storage/app/public/${controller.clientProfile!.content!.first.client_photo}",
                                      ...
                                      Column(
                                        children: [
                                          ...
                                          Container(
                                            ...
                                            child: CustomButtonWidget(buttonText: 'Savings'.tr, 
                                            onTap: () async {

                                              // final AuthController authController = Get.find<AuthController>();
                                              final savingsController = Get.find<SavingsController>();

                                              final rawString = '${controller.clientProfile!.content!.first.id}';
                                                
                                              final parsedId = int.tryParse(rawString);
                                              print('now parsing $parsedId');


                                            if (parsedId == null) {
                                                  // Handle error, e.g., show a message or return early
                                                  Get.snackbar('Error', 'Invalid ID: $rawString');
                                                  return;
                                                } 

                                                await savingsController.fetchSavingsById(parsedId);


                                              // Get.to(ClientSavingsPage(clientId: controller.clientProfile!.content!.first.id!));
                                              
                                              // 1) fetch the details for that account
                                                // await savingsController.fetchSavingsById('${controller.clientProfile!.content!.first.id}' as int);
                                          ...
                                          //     ],
                                          //   ),
                                          //   child: CustomButtonWidget(buttonText: 'View Membership'.tr, 
                                          //   onTap: () async {

                                              
                                          // // 1) Try to get membership by client ID
                                          ...
                                          //         return;
                                          //       } 
                                          //   _checkOrCreateMembership(membershipCtrl, parsedId);
                                          //       // await savingsController.fetchSavingsById(parsedId);
...
Widget clientLoanPaymentHistory() {
  ...
  void showPayLoanDialog() {
   ...
   //---------------------------------------------------------------------------------------
  //  Check or create membership, then show detail
  //---------------------------------------------------------------------------------------
  Future<void> _checkOrCreateMembership(MembershipController membershipCtrl, int clientId) async {
    // Attempt to get membership by ID (assuming membership ID == clientId, or you adjust logic)
    _showLoading();

    await membershipCtrl.getMembershipById(clientId); 

    if (membershipCtrl.selectedMembership == null) {
      // Means not found or 404 from API, so let's create a new membership
      // Provide your default or sample data for membership creation
      await membershipCtrl.createMembership(
        userId: 1, // Adjust to real userId or agent ID
      ...
      // After creation, let's re-fetch or get the newly created membership
      // Typically, we might parse the ID from the "createMembership" response
      // but here's a simplified approach if your API sets membership ID == client
      await membershipCtrl.getMembershipById(clientId);
    }
    ...
    if (membershipCtrl.selectedMembership == null) {
      // Still no membership found or created => handle error
      Get.snackbar('Membership Error', 'Unable to find or create membership');
    } else {
      // Navigate to Membership Detail Screen
  ...
  void _showLoading() {
    setState(() {
      // If you have a local bool for loading, you can do that
    });
  }
