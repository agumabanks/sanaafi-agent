...
import '../../clients/screens/viewClient.dart';
import '../../loans/loans/domain/clientLoanspayHistory.dart';
import '../../loans/loans/domain/loan_offermodal.dart';
import '../../loans/loans/domain/userLoan.dart';
...
Future<void> fetchClientLoansHistory(int clientId) async {
  ...
  Future<void> fetchLoanOffers() async {
    _isLoading = true;
    update();
    final url = 'https://app.sanaa.co/api/v1/loan-plans';
    try {
  ...
  Future<void> fetchUserLoans() async {
    final url = 'https://app.sanaa.co/api/v1/loan-lists/1';
    try {
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
